...
int main (int argc, char *argv[])
{
  ...
  while (pos < flen)
  {
	...
	for (i=0;i<blocklength;i++) {  
		...
		if (lep_curr_sample != lep_last_sample) {
			...
			if (lep_last_sample != -1) lepb(lep_last_sample, (int)(0.5 +(lep_count * 1000000.0 / SAMPLERATE / UNIT))); // us
  ...
  lepb(lep_last_sample, (int)(0.5 +(lep_count * 1000000.0 / SAMPLERATE / UNIT))); // us
